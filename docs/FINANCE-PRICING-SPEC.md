...
> 状态：**§1–§9 已定案（2026-09-16），实现未开始；§10 额度触达检测与窗口归因
> 已定案并落地（2026-09-19，dsh-finance 0.4.19 / dsh-finance-client 0.5.47）**。
> 本文是该领域的**唯一规范源**——
...
9. **折扣价取折后值**：厂商页面用删除线标注永久折扣时（MiniMax `~~4.20~~ 2.10`），
   产物写**折后价**（实际计费值）。取原价会整体虚高一倍。
10. **serviceTier 不混入**：Batch / Flex / Fast / Priority 等**服务档**是独立维度，不属本轮的
    长度阶梯。厂商页面用 Tab / 分表并列时只取 **Standard**（MiniMax 优先档 1.5×、
    OpenAI Batch 半价 —— 混入即整体偏差 50%）。
11. **档位表的形状约束**（阶梯价生成物专用，闸门 A6 逐项断言）：
...
12. **不可达源保留既有 key**（INV-7 在阶梯价上的延伸）：某家源不可达时，该 provider 的
...
**拆分卡的呈现纪律（2026-09-21 用户裁决三条）**：

1. **分界线取该模型价表的最小档，禁止写死阈值**。用户原话：「128K 太小了，几乎随便
   一个任务就能超过」。实测 7 家真实价表有 **32k / 128k / 200k / 256k / 272k / 512k**
   六种档位 —— 写死 128k 会让 256k 档的模型永远显示「界外输入占 ~100%」，而那个
   界外根本不是它的分界。阈值落在**单元格**里（`超过 {tokens} 的输入占 {pct}`），
   列头只说「超出最小档的输入」。分界线由 `smallestTierCeiling()` 给出，
   与 `splitEstimate` 内部的"最小档"**同源**（否则会出现"列头说 >256k、金额按 32k
   折算"的自相矛盾）。
2. **只渲染命中阶梯价的模型**。用户原话：「此处改为仅渲染我们已知支持梯度上下文
   size 的模型，没命中的模型没必要展示在这里」。原先"有用量但没填阶梯价"的模型也
   列出来，除模型名外整行只有一个「—」，纯噪声（实测 5 行里 4 行是破折号）。
   - 判据是 `tierGroupFor(...).status !== 'none'`（**有价表**），**不是**
     `outcome.status === 'ok'`：币种不匹配 / 生效窗口不覆盖仍属"我们确实知道它有
     梯度价"，**必须展示并说明原因** —— 静默不展示 = 静默失败（§2.3 规则 5）。
   - 一张有价的表都没有时整卡给 `EmptyState`（`contextEmpty`），不留空壳。
3. **列头必须自解释**。原「上限可省」被用户判为「无法明确表意」，改为
   「压进最小档可省」（en: `Saved if pushed to smallest tier`）；
   内嵌表标题同步改为「拆小会话能省多少」。
...
| 池 | 成员 | 列 / 操作 |
|---|---|---|
| 订阅计划 | billingMode = plan，或 free（2026-09-19 二次修订：免费收归本卡，**强制月费 0**） | 厂商名、月费、**按量等价**（按量等价金额；省额见下方「表格 tag 纪律」）；Action 列「…」菜单：改标记 / 填·编辑月费 / 移除套餐（free 行只留改标记） |
| 按量付费 | billingMode = metered（free 不进本池） | 厂商名、余额、本月支出；Action 列「…」菜单：改标记 / 手动余额（INV-9） |

**表格 tag 纪律**（2026-09-21 用户裁决，真宿主实测驱动）：**供应商总表内只允许「付费类型」一个 tag**。

起因：额度触达 pill 文案最长 170px（`额度触达 2 次 · 3h12m 后重置`），而供应商列在
531px 卡宽下只有 140px —— pill 溢出 30px 被 `overflow:hidden` 裁掉，并把行高从 50px
撑到 65px，整张表看起来错位。同时付费类型列（72px）里只有一个 40px 的 Tag，空着约 32px。

处置：
- **表内移除三处 tag**：额度触达（→ 详情弹窗）、「超值」（→ 悬浮 `planSaved`）、
  「手填」（→ 悬浮 `manualBalanceLabel`）。付费类型列收到 56px，省下的宽度给其余列。
- 判定原则：**表格列宽是稀缺资源；tag 只在它本身是"分类值"时留在表内**
  （付费类型符合），**"修饰/附加事实"一律降级为悬浮或移进详情弹窗**。
...
但 UI 必须与自动获取值可区分 —— 2026-09-21 起**只走悬浮提示**（表内角标已随上条移除），
权威正文是详情弹窗的「手动余额」行。
...
- **窗口归因的时长**：新增 `financeRateHourly`（每模型 × UTC 小时，**新 key、`stateVersion: 1`**），
  与 `financeRate` 同一套事件语义，只多一个时间维度。独立 unit 的理由同上：
  给 `financeRate` 加维度必须 bump 其 `stateVersion`（现为 2），代价是全量重放。
...
- **不新增表格列**，且 **2026-09-21 起表格内不再挂 Pill**（用户裁决：「非付费类型列的 tag
  一律移除」）。额度信号**收敛进已有的供应商详情弹窗**：摘要行（hits + 重置倒计时/原文）
  + 逐条 episode 明细。
  - 摘要行沿用原 pill 的三态文案，逐字不变：有 `resetAtMs` →
    `额度触达 2 次 · 3h12m 后重置`；无 → `额度触达 2 次 · 重置 <原文>`；都没有 → `额度触达 2 次`。
  - `data-testid="finance-quota-{provider}"` **保留在摘要行上**（断言面不变，只是位置从表格
    移到了弹窗）。
- **改动的理由**（不是口味）：pill 最长 170px，供应商列 140px → 溢出被裁 + 行高 50→65px，
  整张表看起来错位。详见 §5.4「表格 tag 纪律」。
- 逐条 episode 明细只读不可编辑；未终止的 episode 标 `重试中`。
- 订阅与按量同一套机制：订阅触达 = 容量不足，按量触达 = 钱包/资源包触底。
- **反例写进 UI**：触达次数 ≠ 额度大小，不同窗口不可比；`unknown` 窗口必须显式显示"窗口未知"。

> **代价（明写）**：额度触达从"扫一眼表格就能看到"降级为"点开详情才看到"。
> 这是用户对该 trade-off 的显式裁决（修布局优先于表内可见性），SPEC 据此修订。
...
- 历史回填：`financeQuota` / `financeRateHourly` 均 **forward-only**，旧会话不重放补造。

---

### 10.8 窗口归因与双值性价比（2026-09-19 定案并落地）

**动机**（用户原话）：*"既然可以拿到撞墙信号，为何不能统计每次撞墙时最近一个周期
（5 小时、一周、一月）的实际用量？比如今天撞墙可能撞的是 5 小时墙，那我正好想看下
5 小时到底用了哪些模型、用在了哪里、用了多少、用了多久。"*

**归属**：从 §10 的"触达记录"升级为独立的**窗口归因 Card**（追加在「本月值不值」子页），
触达 Pill 退化为入口。数据层与 §10.1–10.4 完全共用。

**窗口切片**：`FinanceLedger.windows`，三个窗口一次切好（客户端不重算）：
...
**决策 D1（名义长度，非自然周期）**：月按固定 30 天，不按自然月天数。理由：这些值最终
带「估」角标，而"逐月不同天数"会让同一个 5 小时窗口在不同月份给出不同数字，用户会以为
算错了——**可解释性优先于虚假精度**（与 INV-3 同精神）。宿主 `WINDOW_SPANS` 与客户端
`NOMINAL_MONTH_MS` 必须逐字一致。

**决策 D2（锚点）**：默认锚在"此刻回溯"（`endMs = nowMs`）；额度触达只是**可选锚点**
（`windowAnchors`），用于回答"那次撞墙前的 5 小时"。锚定态由 `anchoredAtHit` 标出。

**决策 D3（free 套餐）**：`free` 的月费强制为 0，性价比比值会退化为除零。
**不参与双值性价比**——`windowPlanVerdict` 对 `monthlyMicros <= 0` 返回
`plannedMicros/savingsMicros/valueRatio = null`，UI 显示"未填月费，无法算出性价比"。

**双值性价比（用户 2026-09-19 裁决，本节的核心口径）**：

> *"两个值很重要：一个是窗口估价，这个价格接近订阅月费的周期均值；一个是按量等价，
> 这个价格是换算为按量的估算值，二者的差值可以为用户提供明确的『性价比』相对标准。"*

```
窗口估价   = 订阅月费 × (窗口长度 ÷ 30 天)          ← 周期均值
按量等价   = 窗口内实际用量 × 目录价                ← 换算为按量
性价比差值 = 按量等价 − 窗口估价                     ← 正 = 这个窗口赚了
性价比比值 = 按量等价 ÷ 窗口估价
```

- 这与既有 `planInsight`（月度 `equivalentMicros` / `savingsMicros` / `discountRate`）
  是**同一个公式、不同分母**，因此月度卡与窗口卡的数字天然自洽，**不需要两套口径**。
- 订阅厂商的金额是**目录价等价、不是现金流**（§5.4 既有口径）；只在脚注注明来源，
  不另立一套算法（2026-09-19 修正：此前设计稿曾把它写成"另一口径需要分开讲"，属过度设计）。
- 跨多厂商窗口：以**花费最高的订阅厂商**月费作参照（`dominantPlanFee`）。拿用量最大的
  那家月费去比别家的量会得出假结论。

**时长口径**：来自 `financeRateHourly`（模型 × UTC 小时）。旧会话无该键 → 时长为 0，
UI **不渲染时长列**（不是显示 0）。归因小时以 `step/start` 时刻为准（跨小时整步归到开始小时）。

**INV-10 同样适用**：`windows` 与 `quota` 都是同一批观测的另一种切法，
**不参与任何金额口径的累加**，也不得回写 `plans[].quotaTokens`。

**UI 形态**（`QuotaWindowCard`）：窗口切换 SegmentedControl + 三个 Stat
（按量等价 / 窗口估价 / 用量）+ 按模型的用量/时长/金额表。

**性价比结论的落位（2026-09-21 用户裁决）**：结论句（「比订阅估价省/多花 N」）
**渲染为「按量等价」那个数值的 `description` 小字**（ui-kit `Stat.description`，
11px / `--spk-label-3`），与它描述的数字同卡；**不再单独占一行正文**。

理由：这句话说的就是「按量等价」与订阅估价的关系。原先它作为正文段落飘在三个指标卡
**下方**，与指标区割裂、视觉上"格格不入"，读者还得自己把句子与上面的数字对上
（用户原话：「这行文本描述和其他内容格格不入」）。

- 无月费 → 结论**不传 description**（不是显示一句"未填月费"）。
- `Stat.description` 是 ui-kit 的通用槽位（可选），其他指标不用它。

**文案纪律（2026-09-20 用户裁决，硬约束）**：本卡**不承载任何口径解释或提示性文案**——
月长折算口径、"旧会话没有时长数据"这类迁移边界、"未填月费无法算性价比"这类说明
**一律不进 UI**。理由是一贯口径「提示文案只在异常且必须给原因时出现」：无数据本身已由
「—」表意，再补一段解释只是把认知负担丢给用户。

- 无月费 → 性价比结论**整块不渲染**（不是显示一句"未填月费"）。
- 旧会话无时长 → **不渲染时长列**（不是显示"无数据"）。
- 口径说明的唯一归宿是**本文档**（§10.8 与 §5.4 的"目录价等价非现金流"），不是屏幕。
- 反面断言由 `tests/panel.test.tsx` 的「QuotaWindowCard 不含提示性文案」锁死
  （已做负向验证：重新加回文案即 FAIL）；结论的**落位**另由
  「结论句渲染在数值下方（Stat.description）」一例锁死（含 `not.toContain('windowVerdict')`）。

**验收**：`tests/quota-windows.test.ts`（窗口边界 / 锚定 / 时长退化）+ 现有 A11 正交性。
