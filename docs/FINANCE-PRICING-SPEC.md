...
### 5.4 三池分类（本月页）与手动余额（2026-09 定案）

**背景**：计费方式打标入口曾只挂在「按量付费」卡且被 `supportsBalanceFetch` 白名单过滤，
无余额接口的厂商永远无法被打成订阅 → 订阅计划卡成为死表。

**三池规则**（按 provider 生效计费方式折叠后归类，优先级：显式标记 > 已有月费条目 > 宿主默认）：

| 池 | 成员 | 列 / 操作 |
|---|---|---|
| 待定 | 未打标且**无宿主元数据**（`hostMeta` 缺失 = 类型未知；宿主已知厂商按默认进对应池，锁定的标记只读） | 抽屉默认收起（计数徽标）；每行打标：订阅 / 按量 / 免费，保存即落入对应池 |
| 订阅计划 | billingMode = plan | 厂商名、月费、**按量等价节省**（按量等价 − 月费，> 0 加「超值」tag）、编辑 / 移除 |
| 按量付费 | billingMode = metered（free 不得进本池） | 厂商名、余额、本月支出 |
...
**免费（free）**：保留为待定池第三个选项；free 厂商不产生余额视图、不进订阅/按量池，仅在待定池打标后以只读标签呈现。
