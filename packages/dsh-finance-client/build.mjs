...
const cssModulesPlugin = () => ({
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /.*/, namespace: 'css-mod' }, (args) => {
      ...
      // CSS class names must start with a letter: a hex digest may begin with a
  // digit, which makes the generated selector invalid and the browser drops it.
...
// 产物必须是 .cjs：本包 "type": "module"（同 github-ui 的教训）。
await buildAtomic({
