...
test('sourceSparkId is retired (v2 P10/E4): never written, reads as null', () => {
  const core = makeCore()
  const created = core.put(input('Plain', 'note', { sourceSparkId: 'spark-7' }))
  assert.ok(created.sourceSparkId == null, 'the reverse link is no longer written')
  const updated = core.update(created.id, { title: 'Plain v2' })
  assert.ok(updated.sourceSparkId == null)
})
