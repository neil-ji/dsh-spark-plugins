...
export interface MemoryRecord {
  ...
  sourceTurn?: number
  /** @deprecated v2 P10/E4: spark→memory reverse link retired; kept only for reading legacy records. */
  ...
  searchTerms?: string[]
  /**
   * Model-scoped applicability ("provider/model", e.g. "tencent/hy4-preview").
   * Empty/absent = model-agnostic (any model may recall it); non-empty = the
   * memory is a per-model error-notebook entry that may only be auto-injected
   * when the running session model matches one of these ids. Manual
   * memory_search ignores this gate (explicit recall stays unfiltered).
   */
  modelIds?: string[]
...
export interface MemoryListQuery {
  ...
  status?: MemoryStatus
  tag?: string
  /** Only records whose modelIds include this exact "provider/model" id. */
  modelId?: string
...
export interface MemoryPutInput {
  ...
  searchTerms?: string[]
  /** Model-scoped applicability ("provider/model"); empty array clears the gate. */
  modelIds?: string[]
...
export interface PendingCandidate {
  ...
  targetId?: MemoryId
  /**
   * Observation (on-probation) deadline: present when the record already has a
   * future expiresAt. Such candidates are read-only status rows — the engine
   * auto-archives them when the deadline passes (or auto-cancels on citation),
   * so the UI shows remaining days and offers no resolve button.
   */
  expiresAt?: number
