...
import type { SparkView, SparkScope, SparkCrystallize, ProposalView, ScriptView } from 'dsh-spark-wire'
...
type Tab = 'sparks' | 'proposals' | 'scripts' | 'graph'
...
export class SparkController {
  ...
  private ensureLive(): void {
    if (this.sparkUnsub === null) { this.store.update(s => { s.live = true }); this.sparkUnsub = this.api.subscribe(() => { void this.loadAsync() }) }
    if (this.proposalUnsub === null) { this.proposalUnsub = this.api.subscribeProposals(() => { void this.loadAsync() }) }
    if (this.scriptUnsub === null) { this.scriptUnsub = this.api.subscribeScripts(() => { void this.loadAsync() }) }
  }
  capture(input: { title: string; content: string; scope: SparkScope; tags: string[] }): Promise<SparkView> {
    return this.api.capture({ title: input.title, content: input.content, scope: input.scope, tags: input.tags, workspacePath: null, sourceSessionId: 'web-ui', sourceAgentId: null, sourceTurn: null })
  }
  ...
  async crystallize(id: string, opts: SparkCrystallize = { kind: 'insight', importance: 0.5, globalProven: false }): Promise<CrystallizeResult> { const result = await this.api.crystallize(id, opts); await this.loadAsync(); return result }
  async reflect(): Promise<ReflectResult> { const result = await this.api.reflect({}); await this.loadAsync(); return result }
  async resolveProposal(id: string, status: 'accepted' | 'dismissed'): Promise<ProposalView> { const result = await this.api.resolveProposal(id, status); await this.loadAsync(); return result }
  async invokeScript(id: string): Promise<Awaited<ReturnType<SparksApi['invokeScript']>>> { return await this.api.invokeScript(id) }
  async recordScriptResult(id: string, success: boolean): Promise<ScriptView> { const result = await this.api.recordScriptResult(id, success); await this.loadAsync(); return result }
...
// Phase 7: Graph edge kinds.
type GraphEdgeKind = 'crystallized' | 'proposal-link' | 'proposal-cluster'
...
interface NodePos { id: string; x: number; y: number; spark: SparkView }

function buildGraphLayout(sparks: SparkView[], proposals: ProposalView[]): { nodes: NodePos[]; edges: GraphEdge[]; width: number; height: number } {
  // Simple circular layout (Phase 7 MVP). Phase 7.5+: real force-directed sim.
  ...
  const nodes: NodePos[] = activeSparks.map((spark, i) => {
    const angle = activeSparks.length === 0 ? 0 : (i / activeSparks.length) * Math.PI * 2
    return { id: spark.id, x: cx + Math.cos(angle) * radius, y: cy + Math.sin(angle) * radius, spark }
  })
  const nodeById = new Map(nodes.map(n => [n.id, n] as const))
  const edges: GraphEdge[] = []
  for (const p of proposals) {
    if (p.status !== 'pending') continue
    if (p.type === 'link' || p.type === 'cluster') {
      for (let i = 0; i < p.sparkIds.length; i++) {
        for (let j = i + 1; j < p.sparkIds.length; j++) {
          const a = nodeById.get(p.sparkIds[i]!)
          const b = nodeById.get(p.sparkIds[j]!)
          if (a !== undefined && b !== undefined) {
            edges.push({ kind: p.type === 'link' ? 'proposal-link' : 'proposal-cluster', fromId: a.id, toId: b.id, label: p.explanation })
          }
        }
      }
    }
  ...
  for (const spark of activeSparks) {
    if (spark.crystallized !== null) {
      edges.push({ kind: 'crystallized', fromId: spark.id, toId: spark.crystallized.hippoId, label: 'hippo:' + spark.crystallized.kind })
    }
  }
...
function Loaded({ injected }: { injected: SparkSectionInjected }): ReactNode {
  ...
  const filteredSparks = useMemo<SparkView[]>(() => { return sparkFilter === 'all' ? state.sparks : state.sparks.filter(s => s.status === sparkFilter) }, [state.sparks, sparkFilter])
  const filteredProposals = useMemo<ProposalView[]>(() => { return state.proposals.filter(p => p.status === proposalFilter) }, [state.proposals, proposalFilter])
  const filteredScripts = useMemo<ScriptView[]>(() => { return scriptFilter === 'all' ? state.scripts : state.scripts.filter(s => s.scope === scriptFilter) }, [state.scripts, scriptFilter])

  const onSubmit = useCallback(async (): Promise<void> => {
    if (formTitle.trim().length === 0 || formContent.trim().length === 0) { setFormError(t('captureFailed')); return }
    ...
    } catch (error) { setFormError(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }, [controller, formTitle, formContent, formTags, formScope, t])

  const onArchive = useCallback(async (id: string): Promise<void> => { setBusy(true); try { await controller.archive(id) } finally { setBusy(false) } }, [controller])
  const onDelete = useCallback(async (id: string): Promise<void> => {
    if (typeof window !== 'undefined' && window.confirm !== undefined) { if (!window.confirm(t('deleteConfirm'))) return }
    setBusy(true); try { await controller.delete(id) } finally { setBusy(false) }
  }, [controller, t])
  const onCrystallize = useCallback(async (spark: SparkView): Promise<void> => {
    if (typeof window !== 'undefined' && window.confirm !== undefined) { if (!window.confirm(t('crystallizeConfirm'))) return }
    setBusy(true); setCrystallizeError(null)
    try {
      ...
      if (err.code === 'SPARK_HIPPO_UNAVAILABLE') setCrystallizeError(t('crystallizeHippoUnavailable'))
      else setCrystallizeError(err.message || t('crystallizeFailed'))
    } finally { setBusy(false) }
  }, [controller, t])
  const onReflect = useCallback(async (): Promise<void> => {
    setBusy(true); setReflectInfo(null)
    try {
      const result = await controller.reflect()
      setReflectInfo(t('reflectDone').replace('X', String(result.newProposals.length)).replace('Y', String(result.skippedDuplicate)))
      setTimeout(() => setReflectInfo(null), 6000)
    } catch (error) { setReflectInfo(error instanceof Error ? error.message : t('reflectFailed')) } finally { setBusy(false) }
  }, [controller, t])
  const onResolve = useCallback(async (id: string, status: 'accepted' | 'dismissed'): Promise<void> => { setBusy(true); try { await controller.resolveProposal(id, status) } finally { setBusy(false) } }, [controller])
  const onInvokeScript = useCallback(async (id: string): Promise<void> => { setBusy(true); try { await controller.invokeScript(id) } finally { setBusy(false) } }, [controller])
  const onRecordScriptResult = useCallback(async (id: string, success: boolean): Promise<void> => { setBusy(true); try { await controller.recordScriptResult(id, success) } finally { setBusy(false) } }, [controller])
  const onDeleteScript = useCallback(async (id: string, name: string): Promise<void> => {
    if (typeof window !== 'undefined' && window.confirm !== undefined) { if (!window.confirm(t('scriptDeleteConfirm').replace('{name}', name))) return }
    setBusy(true); try { await controller.deleteScript(id) } finally { setBusy(false) }
  }, [controller, t])
      ...
      <div className="spark-toolbar">
        ...
        <div className="spark-toolbar-right">
          ...
          {tab === 'sparks' ? (
            ...
            <Button variant="primary" disabled={busy} onClick={() => { void onReflect() }}>{t('reflect')}</Button>
          )}
        </div>
      </div>

      {tab === 'sparks' ? (
        <SparksTab sparks={filteredSparks} totalSparks={state.sparks.length} showForm={showForm}
          formTitle={formTitle} setFormTitle={setFormTitle} formContent={formContent} setFormContent={setFormContent}
          formTags={formTags} setFormTags={setFormTags} formScope={formScope} setFormScope={setFormScope}
          formError={formError} crystallizeError={crystallizeError} crystallizeInfo={crystallizeInfo}
          sparkFilter={sparkFilter} setSparkFilter={setSparkFilter} busy={busy} t={t}
          onSubmit={onSubmit} onArchive={onArchive} onDelete={onDelete} onCrystallize={onCrystallize} />
      ) : tab === 'proposals' ? (
        <ProposalsTab proposals={filteredProposals} totalProposals={state.proposals.length}
          reflectInfo={reflectInfo} proposalFilter={proposalFilter} setProposalFilter={setProposalFilter}
          busy={busy} t={t} onResolve={onResolve} />
      ) : tab === 'scripts' ? (
        <ScriptsTab scripts={filteredScripts} totalScripts={state.scripts.length}
          scriptResultInfo={scriptResultInfo} scriptFilter={scriptFilter} setScriptFilter={setScriptFilter}
          expandedScriptId={expandedScriptId} setExpandedScriptId={setExpandedScriptId}
          busy={busy} t={t} onInvoke={onInvokeScript} onRecordResult={onRecordScriptResult} onDelete={onDeleteScript} />
      ) : (
        <GraphTab sparks={state.sparks} proposals={state.proposals} t={t} />
      )}
    </div>
  )
}
...
interface SparksTabProps {
  sparks: SparkView[]; totalSparks: number; showForm: boolean
  ...
  formError: string | null; crystallizeError: string | null
  ...
  onSubmit: () => Promise<void>; onArchive: (id: string) => Promise<void>
  onDelete: (id: string) => Promise<void>; onCrystallize: (spark: SparkView) => Promise<void>
}
...
interface ProposalsTabProps {
  proposals: ProposalView[]; totalProposals: number; reflectInfo: string | null
...
function ProposalsTab(props: ProposalsTabProps): ReactNode {
  ...
  return (
    <div>
      ...
      {reflectInfo !== null ? (
        <div className="spark-error" style={{ color: 'var(--dsw-alias-state-success-primary, var(--dsw-static-green-500))', borderColor: 'var(--dsw-alias-state-success-primary, var(--dsw-static-green-500))' }}>{reflectInfo}</div>
      ) : null}
      {proposals.length === 0 ? (
...
interface ScriptsTabProps {
  scripts: ScriptView[]; totalScripts: number; scriptResultInfo: string | null
  ...
  return (
    <div>
      ...
      {scriptResultInfo !== null ? (
        <div className="spark-error" style={{ color: 'var(--dsw-alias-state-success-primary, var(--dsw-static-green-500))', borderColor: 'var(--dsw-alias-state-success-primary, var(--dsw-static-green-500))' }}>{scriptResultInfo}</div>
      ) : null}
      {scripts.length === 0 ? (
...
/** Phase 7: Graph tab — circular SVG layout with crystallized/proposal edges. */
interface GraphTabProps {
  sparks: SparkView[]; proposals: ProposalView[]; t: Translate
}

function GraphTab(props: GraphTabProps): ReactNode {
  const { sparks, proposals, t } = props
  const active = sparks.filter(s => s.status === 'active')
  const layout = useMemo(() => buildGraphLayout(sparks, proposals), [sparks, proposals])
  if (active.length === 0) return <div className="spark-empty">{t('graphEmpty')}</div>
  const nodeById = new Map(layout.nodes.map(n => [n.id, n] as const))
  return (
    <div>
      <svg width={layout.width} height={layout.height} style={{ display: 'block', margin: '0 auto', background: 'var(--dsw-alias-bg-layer-2)', borderRadius: 12, border: '1px solid var(--dsw-alias-border-l1)' }}>
        {layout.edges.map((edge, i) => {
          const a = nodeById.get(edge.fromId)
          const b = nodeById.get(edge.toId)
          if (a === undefined || b === undefined) return null
          const isGhost = edge.kind !== 'crystallized'
          // For crystallized, b is a hippo id not in our nodes — render as edge to canvas edge.
          const toX = edge.kind === 'crystallized' ? layout.width - 20 : b.x
      ...
      <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 12, fontSize: 12, color: 'var(--dsw-alias-label-secondary)' }}>
        <span><span style={{ display: 'inline-block', width: 24, height: 0, borderTop: '2px solid var(--dsw-alias-state-success-primary, var(--dsw-static-green-500))', verticalAlign: 'middle', marginRight: 6 }} />{t('graphEdgeCrystallized')}</span>
        <span><span style={{ display: 'inline-block', width: 24, height: 0, borderTop: '1.5px dashed var(--dsw-alias-state-warn-primary, var(--dsw-static-deepseek-450))', verticalAlign: 'middle', marginRight: 6 }} />{t('graphEdgeProposal')}</span>
      </div>
    </div>
  )
}

export function bindSparkController(controller: SparkController): { api: SparksApi; useSnapshot: SnapshotSelectorHook<SparkState>; controller: SparkController } {
