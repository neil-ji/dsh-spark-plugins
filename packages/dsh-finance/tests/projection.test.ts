...
describe('financeUsage projection', () => {
  ...
  it('replaces rather than double-counts the same step', () => {
    ...
    expect(projection.wire.view(state).totals).toEqual({ uncachedInputTokens: 120, cacheReadTokens: 0, cacheWriteTokens: 0, outputTokens: 60 })
  })
  ...
  it('ignores a duplicate identical report for the same step', () => {
    ...
    const once = projection.wire.view(state)
    state = projection.apply(state, event)
    const twice = projection.wire.view(state)
  ...
  it('accumulates distinct steps in the same turn', () => {
    ...
    expect(projection.wire.view(state).totals).toEqual({ uncachedInputTokens: 300, cacheReadTokens: 0, cacheWriteTokens: 0, outputTokens: 150 })
  })
  ...
  it('does not count usage before any request/header', () => {
    let state = projection.init()
    state = projection.apply(state, usageChunkEvent(T0, 1, 0, { inputTokens: 100, outputTokens: 50 }))
    expect(projection.wire.view(state).totals.uncachedInputTokens).toBe(0)
  })
  ...
  it('defends against zero/negative usage values (clamped by schema at rest)', () => {
    ...
    expect(projection.wire.view(state).totals).toEqual({ uncachedInputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, outputTokens: 0 })
  })
})
...
describe('financeUsageHourly projection', () => {
  ...
  it('accumulates usage by model and by UTC hour', () => {
    ...
    const view = hourlyProjection.wire.view(state)
  ...
  it('ignores a duplicate identical report for the same step', () => {
    ...
    const once = hourlyProjection.wire.view(state)
    state = hourlyProjection.apply(state, event)
    expect(hourlyProjection.wire.view(state)).toEqual(once)
  })
  ...
  it('does not count usage before any request/header', () => {
    let state = hourlyProjection.init()
    state = hourlyProjection.apply(state, usageChunkEvent(T0, 1, 0, { inputTokens: 100, outputTokens: 50 }))
    expect(hourlyProjection.wire.view(state).byModelHour).toEqual({})
  })
