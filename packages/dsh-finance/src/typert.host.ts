    ...
    services: [
      {
        ...
        types: [
          ...
          { name: 'FinanceLedger', declaration: 'export interface FinanceLedger { generatedAt: number; currency: string; totals: FinanceTokenBuckets; totalCostMicros: number; sessionCount: number; workspaceCount: number; taskCount: number; windowedSinceMs: number | null; byDay: readonly FinanceDayRow[]; byModel: readonly FinanceModelRow[]; byWorkspace: readonly FinanceWorkspaceRow[]; tasks: readonly FinanceTaskRow[]; sessions: readonly FinanceSessionRow[]; byHourOfDay: readonly FinanceHourOfDayRow[]; peakValley: FinancePeakValleySplit; }' },
