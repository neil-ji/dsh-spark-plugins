 ...
 *
 * 三类边（全部来自既有数据，不引入新存储）：
 *  1. `crystallized`：火花 → 它结晶出的记忆（`record.crystallized.hippoId`），即结晶谱系；
 *  2. `tag`：两条火花共享标签数 ≥ tagMinShared（默认 2），权重=共享数；
 *  3. `proposal`：涌现提议里被判为关联的火花对（`proposal.sparkIds`），
 *     权重=同现的提议条数 —— 这是「引擎认为它们相关」的证据，与标签相似度正交。
 *
 * 裁剪策略：只取最近活跃的 `limit` 条火花（archived 排在最后），避免图被历史
 * 库存淹没；`truncated` 告知前端「图不是全量」。墓碑（deletedAt）与 dropped
 * 状态不参与 —— 它们已经是"判过死刑"的记录。
 */
import type {
  ProposalView,
  SparkGraph,
...
const STATE_RANK: Record<SparkView['inboxState'], number> = {
  pending: 0,
  crystallized: 1,
  archived: 2,
  dropped: 3,
...
export function memoryNodeId(hippoId: string): string {
  return MEMORY_PREFIX + hippoId
}
...
export function selectGraphSparks(
  ...
  const alive = sparks.filter((spark) => spark.deletedAt === null && spark.inboxState !== 'dropped')
  const ordered = [...alive].sort((a, b) => {
    const rank = STATE_RANK[a.inboxState] - STATE_RANK[b.inboxState]
...
export function buildSparkGraph(
  ...
  // 1) 结晶谱系：火花 → 记忆节点（每个结晶火花带出唯一一个记忆节点）。
  for (const spark of picked) {
    if (spark.crystallized === null) continue
    const memoryId = memoryNodeId(spark.crystallized.hippoId)
    if (!nodes.some((node) => node.id === memoryId)) {
      nodes.push({
        id: memoryId,
        kind: 'memory',
        label: spark.title,
    ...
    pushEdge(sparkNodeId(spark.id), memoryId, 'crystallized', 1)
  }

  // 2) 标签亲和：共享 ≥ tagMinShared 个标签的火花两两相连。
